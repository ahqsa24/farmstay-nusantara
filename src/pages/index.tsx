      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-farm-beige/90 border-b border-farm-border/50 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-serif text-2xl font-bold tracking-tight text-farm-green group-hover:text-farm-green-hover transition-colors">
              {t.common.brand}
            </span>
          </Link>
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            {/* Login button / User profile avatar dropdown */}
            {isMounted && isAuthenticated ? (
              <div className="relative shrink-0" ref={dropdownRef}>
                <button
                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  className="flex items-center gap-1.5 focus:outline-none p-1 rounded-lg hover:bg-farm-border/20 transition-all border border-transparent hover:border-farm-border/40"
                >
                  ...
                  <div className="absolute right-0 mt-2 w-64 origin-top-right rounded-xl bg-white border border-farm-border shadow-xl focus:outline-none z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                    <div className="px-4 py-3.5 border-b border-farm-border/60 bg-farm-cream">
                      <p className="text-[10px] font-black uppercase tracking-wider text-farm-green">
                        {user?.role === "admin" ? (isId ? "Portal Admin" : "Admin Portal") : (user?.role === "owner" ? (isId ? "Pemilik Akomodasi" : "Farmstay Owner") : (isId ? "Pengunjung" : "Visitor Traveler"))}
                      </p>
